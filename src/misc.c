...
void tweedious0(double *w,double *w1,double *w2,double *w1p,double *w2p,
	       double *w2pp,double *y,double *phi,double *p,double *eps,int *n,
               double *th,double *rho,double *a, double *b)
/* This version is original, but suffers from cancellation error problems in derviative 
   evaluation...
   ...
   d logW / dp in w1p
   d2 logW / dp2 in w2p
   d2 logW / dp dphi in W2pp

   if *p<0 on entry then 
   derivatives are w.r.t. working parameters rho=log(phi), and
   th defining p = (a + b * exp(th))/(exp(th)+1). 
   note, 1<a<b<2 (all strict) 
...
*/
{ int j_max,i,j_lo,j_hi,jb,jal,j0,j,ok,work_p=0;
  double x,x1,x2,xx,ymax,ymin,alpha,*alogy,*p1,*p2,*p3,*p4,*p5,
  ...
  if (*p < 0) { /* do everything in terms of working parameters, rho, th */
    work_p = 1;
    *phi = exp(*rho); 
    if (*th>0) { 
      exp_th =  exp(- *th);
  ...
  if (work_p) {
    if (*th>0) { 
      x = (1 - *b) + ( 1 - *a)*exp_th;
      wpp_base = (*b - *a)*exp_th/(x*x);
    } else {
      x = (1 - *b)*exp_th + ( 1 - *a);
      wpp_base = (*b - *a)*exp_th/(x*x);
    }
  } else wpp_base = 1/(*phi * onep2);
  /* initially establish the min and max y values, and hence the initial buffer range,
     at the same time produce the alpha log(y) log(y)/(1-p)^2 and log(y)/(1-p)^3 vectors. */ 
  ...
  *logy1p3 = *logy1p2/onep;
  ...
  for (i=0;i<*n;i++) { /* loop through y */
    ...
    wp1jmin = x * *eps; // TRANSFORM?? 
    wdW2min = fabs(x*x + wp2[j_max] - 2 * j * logy1p3[i]) * *eps; // TRANSFORM??
    Wppmin = fabs(wpp[j_max] + x*exp(wb1[j_max]-jalogy-wmax)) * *eps; // RECHECK??
    /* start upsweep to convergence or end of available buffered values */
    ok = 0;xmax=x1max=x2max=0.0;
    for (j=j_max+j0,jb=j_max;jb<=j_hi;jb++,j++) { // note initially wi etc initialized to 1 and summation starts 1 later
      jalogy = j * alogy[i];
      ...
      wp1j = wp1[jb] - j * logy1p2[i]; /* d log W / dp */
      wp2j = wp2[jb] - 2 * j * logy1p3[i]; /* d^2 log W/ dp^2 */
      if (work_p) { /* transform to working parameterization */
        wp2j = wp1j * dpth2 + wp2j * dpth1 * dpth1; /* d^2 log W/ dth^2 */
        wp1j *= dpth1; /* d log W / dth */
      }
      wj_scaled =  exp(wj-wmax);
      ...
      x=fabs(x);x1=fabs(x1);x2=fabs(x2);
      if (jb==j_max+5) Rprintf("\n%d %.12g %.12g %.12g %.12g %.12g %.12g\n",j_max+j0,wj,w1j,w2j,wp1j,wp2j,wpp[jb]);
      if (x>xmax) {xmax=x;wp1jmin=x * *eps;}
      if (x1>x1max) {x1max=x1;wdW2min=x1 * *eps;}
      if (x2>x2max) {x2max=x2;Wppmin=x2 * *eps;}
    ...
    for (j=j_max-1+j0,jb=j_max-1;jb>=j_lo;jb--,j--) { 
      ...
      if ((wj < wmin)&&(w1j < w1min)&&(w2j < w2min)&&
          (x < wp1jmin)&&(x1 < wdW2min)&&(x2 < Wppmin)) { ok=1;break;} /* converged on downsweep */
    } /* end of downsweep to buffer end */ 
    ...
    while (!ok) { /* while downsweep unconverged need to fill in more buffer */
      for (jb=j_lo-1;jb>=0;jb--,j--) { /* fill buffers and calculate w terms */
        ...
        wdlogwdp += x;    /* sum_j W_j dlogW_j/dp */
        x1 = wj_scaled*(wp1j*wp1j + wp2j);
        wdW2d2W += x1;  /* sum_j W_j* (dlog W_j/dp)^2 + W_j d^2logW_j/dp^2 */  
        x2 = dW_scaled*wp1j + wj_scaled*wpp[jb]; 
        dWpp += x2; /* sum_j dW_j/dphi dlog W_j/dp + W_j d^2logW_j/dpdphi */ 
        x=fabs(x);x1=fabs(x1);x2=fabs(x2);
        if (x>xmax) {xmax=x;wp1jmin=x * *eps;}
    ...
    xx = -exp(w1i-wmax); /* need to re-scale dW/dphi */
